...
function encryptFields(model, data) {
    ...
    for (var _i = 0, _a = ENCRYPTED_FIELDS[model]; _i < _a.length; _i++) {
        var field = _a[_i];
            ...
            switch (_a.label) {
                case 0:
                    ...
                    if (action === 'createMany' && (args === null || args === void 0 ? void 0 : args.data) && model && Array.isArray(args.data)) {
                        args.data = args.data.map(function (row) { return encryptFields(model, row); });
                    }
                ...
                case 1:
                    ...
                    if ((action === 'findUnique' || action === 'findFirst' || action === 'findMany') && result && model) {
                        if (Array.isArray(result)) {
                            return [2 /*return*/, result.map(function (row) { return decryptFields(model, row); })];
                        }
                        else {
                            return [2 /*return*/, decryptFields(model, result)];
                        }
                    }
                    // Decrypt on update
